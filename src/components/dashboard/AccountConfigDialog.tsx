...
export function AccountConfigDialog({ account, open, onOpenChange }: AccountConfigDialogProps) {
  ...
  const calculateDaysRemaining = (balance: any, dailySpend: any) => {
    const bal = parseToNumber(balance);
    const spend = parseToNumber(dailySpend);
    if (spend === 0) return "∞";
    return Math.floor(bal / spend);
  };
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] flex flex-col">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${account.platform === "meta" ? "bg-[#1877F2]/10" : "bg-[#4285F4]/10"
              }`}>
          ...
          <div className="space-y-4">
            ...
            {alertEnabled && (
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Você será alertado quando o saldo ficar abaixo de {formatBRL(minBalanceAlert)}
                </p>
              </div>
            )}
          </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {CAMPAIGN_OBJECTIVES.map((objective) => (
                  <div
                    key={objective.value}
                    className={`flex items-center gap-3 p-3 rounded-lg border transition-colors cursor-pointer ${reportObjectives.includes(objective.value)
