  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    ...
    queryFn: async (): Promise<DashboardStats> => {
      ...
      accountsList.forEach(account => {
        ...
        // Check low balance
        if (account.balance !== null && account.min_balance_alert !== null) {
          if (account.balance <= account.min_balance_alert) {
      ...
      return {
        ...
        totalBalance: accountsList.reduce((sum, a) => sum + (a.balance || 0), 0),
        totalDailySpend: accountsList.reduce((sum, a) => sum + (a.daily_spend || 0), 0),
